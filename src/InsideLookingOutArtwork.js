...
const InsideLookingOutArtwork = () => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        ...
        if (!isCancelled) {
          setDataError("Live UKHSA COVID-19 data is currently unavailable.");
        }
      }
    };
    ...
    p5.draw = () => {
      if (count < coronaStats.data.length) {
        ...
        for (let i = 0; i < coronaStats.data[count].dailyDeaths; i++) {
          const randomPixel = Math.floor(p5.random(0, img.pixels.length));
        ...
        const day = coronaStats.data[count].date.slice(8);
        const month = coronaStats.data[count].date.slice(5, 7);
        const year = coronaStats.data[count].date.slice(0, 4);
  ...
  if (!userInteractionComplete) {
    return (
      ...
      >
        <p>
          Live data from the UKHSA COVID-19 API is used as the source to create a
          narrative structure for this audiovisual web artwork. Each day of the
          Coronavirus pandemic in England is represented by a single frame,
          creating an animated film that tells a story about the impact of the
          virus on the people of England.
        </p>
        <p>
...
export default InsideLookingOutArtwork;